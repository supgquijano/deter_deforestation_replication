...
series.weatherRain <- 
  ...
  dplyr::rename(year = year_prodes) %>% # change column name
  filter(year >= 2001) %>% 
  ungroup()
...
series.weatherTemp <- 
  ...
  mutate(year_prodes = if_else(month < 8, year, year + 1)) %>% # create year prodes column  
  group_by(muni_code, year_prodes) %>% # defines the muni_code + year as the level of interest
  dplyr::summarise(weatherTemp_mean = sum(weatherTemp)/12) %>% # collapse data summing monthly values
  dplyr::rename(year = year_prodes) %>% # change column name
  filter(year >= 2001) %>% 
  ungroup()
...
save(series.weatherRain, file = file.path("data/projectSpecific", "series_weatherRain.Rdata"))

save(series.weatherTemp, file = file.path("data/projectSpecific", "series_weatherTemp.Rdata"))
